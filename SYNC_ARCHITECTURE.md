...
## Phase 7 local-network user interface

Settings now includes an always-visible **Local network sync** section:

- Device-local Disabled, Host, and Client modes.
- Manual HTTP/HTTPS host address entry, compatibility probing through `/.well-known/weektodo-sync`, and desktop LAN discovery.
- Desktop discovery sends the non-secret `WEEKTODO_DISCOVER_V1` UDP broadcast on port 48161. A host responds only with service name, protocol version, display name, and public address.
- Host mode clearly states that the separately deployed sync/database service must already be running; the planner UI does not silently start a database server.
- New-device enrollment, matching approval code, pending approvals, connected-device listing, and revocation use the existing authenticated endpoints.
- Current connection state, last successful synchronization, pending outbox count, manual sync, conflict fields, and error details are visible without opening developer tools.
- Status priority distinguishes authentication failures and conflicts first, then pending work, offline state, host availability, and successful synchronization. When changes are pending offline, both facts are displayed.
- The recovery-backup action reuses the versioned `.wtdb` export path from Phase 6.
- Local mode/address settings never enter the shared settings document, preventing one device from redirecting another device's sync host.
